...
func (w *Wallet) MakeMultiSigScript(addrs []wfcutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*wfcutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*wfcutil.AddressScriptHash, error) {
	var p2shAddr *wfcutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = wfcutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*wfcutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
