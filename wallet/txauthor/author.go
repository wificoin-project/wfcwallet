...
import (
	...
	h "github.com/wificoin-project/wfcwallet/internal/helpers"
...
// InputSourceError.
type InputSource func(target wfcutil.Amount) (total wfcutil.Amount, inputs []*wire.TxIn,
	inputValues []wfcutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []wfcutil.Amount
	TotalInput      wfcutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb wfcutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []wfcutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := wfcutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
