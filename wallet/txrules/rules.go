...
const DefaultRelayFeePerKb wfcutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb wfcutil.Amount) wfcutil.Amount {
	...
	relayFee := wfcutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount wfcutil.Amount, scriptSize int, relayFeePerKb wfcutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb wfcutil.Amount) bool {
	...
	return IsDustAmount(wfcutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb wfcutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > wfcutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb wfcutil.Amount, txSerializeSize int) wfcutil.Amount {
	fee := relayFeePerKb * wfcutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > wfcutil.MaxSatoshi {
		fee = wfcutil.MaxSatoshi
	}
